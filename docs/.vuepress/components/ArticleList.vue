...
<template>
  <div class="article-wrapper">
    <div v-if="!items.length">空空如也.</div>
    ...
    <article v-for="{ info, path } in items" :key="path" class="article" @click="$router.push(path)">
      ...
      <div class="article-info">
        ...
        <span v-if="info.date && !isTimeline" class="date">日期: {{ new Date(info.date).toLocaleDateString() }}</span>

        <span v-if="info.category" class="category">类别: {{ info.category.join(', ') }}</span>
